...
F2 = self(F1, nProgeny = 100) 

##build GS model using F2##

ans = RRBLUP(F2, simParam=SP)
...
F3 = selectFam(F2, 10, use="ebv", selectTop=TRUE) 
...
PYT = selectInd(F5, 20, use="ebv", selectTop=TRUE) 


##set EBV using BLUP model##
PYT = setEBV(PYT, ans2, simParam=SP)
cor5 = cor(gv(PYT),ebv(PYT))

## select top plants from PYT to form advanced yield trial ##

AYT = selectInd(PYT,  20, use="ebv", selectTop=TRUE) 


##set EBV using BLUP model##
AYT = setEBV(AYT, ans2, simParam=SP)
cor6 = cor(gv(AYT),ebv(AYT))

## select top plants to form variety ##
Variety = selectInd(AYT, 1, use="ebv", selectTop=TRUE)
